...
pchisq(2758.3-2158.7, df=3332-3315, lower.tail=FALSE)

pchisq(churn.logit$null.deviance-churn.logit$deviance, 
       df=churn.logit$df.null-churn.logit$df.residual, 
       lower.tail=FALSE)

(2758.3-2158.7)/2758.3
with(churn.logit, (null.deviance-deviance)/null.deviance)
...
pchisq(1969.38, df=1876, lower.tail=FALSE) 

pchisq(deviance(pid.mlogit), df=df.residual(pid.mlogit), lower.tail=FALSE)
...
pid.mlogit.pred <- predict(pid.mlogit, newdata=PID, type="response")
head(pid.mlogit.pred)

pred <- colnames(pid.mlogit.pred)[max.col(pid.mlogit.pred)]
head(pred)

pid.mlogit.pred <- data.frame(pid.mlogit.pred)
pid.mlogit.pred$Actual <- PID$PID
pid.mlogit.pred$Predicted <- pred
head(pid.mlogit.pred)

table(pid.mlogit.pred$Actual, 
      factor(pid.mlogit.pred$Predicted, levels=levels(pid.mlogit.pred$Actual),
             labels=levels(pid.mlogit.pred$Actual)), dnn=c("Actual", "Predicted"))
mean(pid.mlogit.pred$Actual==pid.mlogit.pred$Predicted)
...
boxplot(ys ~ xs, names=levels(fgl.test$type), ylim=c(0, 1), las=1, col=rainbow(6), 
        xlab="Glass Type", ylab="Estimated Probabilities",
        main="Probabilities of Group Membership against True Group")
stripchart(ys ~ xs, method="jitter", pch=21, col="black", bg="gray",
...
boxplot(fgl.mlogit.cv, horizontal=TRUE, col="mistyrose", 
        xlab="Accuracy", main="Accuracy for Forensic Glass (100 samples)")
stripchart(fgl.mlogit.cv, method="jitter", 
           pch=21, col="black", bg="tomato", add=TRUE)
...
# [그림 8-4]
windows(width=7.0, height=5.5)
hist(seizure$sumY, breaks=20, col="cornflowerblue", las=1,
